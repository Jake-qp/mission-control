...
export async function POST(request: NextRequest) {
  ...
  // Gateway state backup (OpenClaw CLI removed — return not-supported)
  if (target === 'gateway') {
    return NextResponse.json({ error: 'Gateway backup via OpenClaw CLI has been removed' }, { status: 501 })
  }
